...
Note: To connect to a SQL database, be sure you have already set up the connection through ODBC Data Sources (Windows). TODO: How is this done on Mac? 
...
Another function that may be useful is `extarct_tibbles` which will put the instrument tibbles into a named list of tibbles where each element of the list is the dataframe for one instrument. Then you can map over the instrument tibbles with `map` and `filter` to only include randomized participant data.
